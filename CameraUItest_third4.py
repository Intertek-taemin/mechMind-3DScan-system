...
# ✅ 환경변수에서 원격 URL(토큰 포함)을 읽어옴
# Windows PowerShell 예:
#   setx GIT_REMOTE_URL "https://user:token@host/repo.git"
# 새 터미널에서 적용됨
GIT_REMOTE_URL_WITH_TOKEN = os.environ.get("GIT_REMOTE_URL", "").strip()
...
class GitUploader:
    def __init__(self, repo_path, remote_url_with_token, branch='main', log_fn=None):
        self.repo_path = os.path.abspath(repo_path)
        self.remote_url = remote_url_with_token
        self.branch = branch
        self.log_fn = log_fn  # ✅ GUI 로그로 보내는 콜백
    ...
    def _mask_url(self, url: str) -> str:
        # https://user:token@host/...  형태면 token 부분 마스킹
        try:
    ...
    def run_command(self, command, check_error=True, show_output=True):
        try:
            ...
            if show_output and output:
                # URL 토큰이 출력에 섞일 여지가 있어서 마스킹
                output_safe = output.replace(self.remote_url, self._mask_url(self.remote_url)) if self.remote_url else output
                self._log(output_safe)

            if check_error and result.returncode != 0:
                if "nothing to commit" not in output and "up to date" not in output.lower():
                    if "already exists" not in output:
                        self._log(f"⚠️ 경고: {output}")
                        return False, output

            return True, output

        except Exception as e:
            self._log(f"❌ 예외 발생: {str(e)}")
            return False, str(e)

    def check_git_installed(self):
        success, _ = self.run_command("git --version", check_error=False, show_output=False)
        if not success:
            self._log("❌ Git이 설치되어 있지 않습니다. https://git-scm.com/ 에서 설치해주세요.")
            return False
        return True
    ...
    def setup_remote(self):
        self._log("🔗 원격 저장소 설정 중...")
        self.run_command("git remote remove origin", check_error=False, show_output=False)

        if not self.remote_url:
            self._log("❌ REMOTE URL이 비어있습니다. GIT_REMOTE_URL_WITH_TOKEN 을 설정하세요.")
            return False

        success, output = self.run_command(f'git remote add origin "{self.remote_url}"', check_error=False)
        if success or "already exists" in output:
            self._log("✅ 원격 저장소 설정 완료")
            self.run_command("git remote -v", show_output=True)
            return True

        self._log("❌ 원격 저장소 설정 실패")
        return False
    ...
    def init_or_clone(self):
        if self.is_git_repo():
            self._log("✅ 기존 Git 저장소 사용")

            if not self.check_remote_exists():
                self._log("⚠️ 원격 저장소가 설정되지 않았습니다.")
                if not self.setup_remote():
                    return False
            else:
                self._log("✅ 원격 저장소 이미 설정됨")

            current_branch = self.get_current_branch()
            if current_branch:
                self._log(f"📌 현재 브랜치: {current_branch}")
                self.branch = current_branch

            return True

        # 폴더에 파일이 있으면 초기화
        self._log("📁 Git 저장소 초기화...")
        ...
        self._log(f"📌 브랜치 설정: {self.branch}")
        return True
    ...
    def pull(self):
        ...
        if not output.strip():
            self._log(f"ℹ️ 원격에 '{self.branch}' 브랜치가 없습니다. 첫 push를 진행합니다.")
            return True

        success, output = self.run_command(f"git pull origin {self.branch} --rebase", check_error=False)
        if "up to date" in output.lower():
            self._log("✅ 이미 최신 상태입니다.")
        elif success:
            self._log("✅ Pull 성공")
        return True
    ...
    def commit(self, message=None):
        ...
        self._log(f"💾 커밋: {message}")

        success, output = self.run_command(f'git commit -m "{message}"', check_error=False)
        if "nothing to commit" in output:
            self._log("ℹ️ 변경사항이 없습니다.")
            return True
        return success

    def push(self):
        self._log(f"⬆️ Push 실행 (브랜치: {self.branch})...")

        if not self.check_remote_exists():
            self._log("⚠️ 원격 저장소가 없습니다. 재설정 중...")
            if not self.setup_remote():
                return False

        success, output = self.run_command(f"git push -u origin {self.branch}", check_error=False)
    ...
    def upload(self, commit_message=None, pull_first=True):
        ...
        if not self.push():
            self._log("⚠️ Push 실패. 재시도 중...")
            if not self.push():
                self._log("❌ Push가 계속 실패합니다.")
                self._log(f"💡 수동: cd {self.repo_path} && git push -u origin {self.branch}")
                return False
...
class GitUploadWorker(QObject):
    log = Signal(str)
    finished = Signal(bool)

    def __init__(self, repo_path, remote_url, branch, pull_first, commit_message):
        super().__init__()
    ...
    def run(self):
        ...
        uploader = GitUploader(
            ...
            remote_url_with_token=self.remote_url,
            branch=self.branch,
            log_fn=_emit
        )
...
class MainWindow(QMainWindow):
    ...
    def on_upload_git_clicked(self):
        ...
        branch = GIT_BRANCH
        pull_first = GIT_PULL_FIRST

        if not remote_url:
            self.append_log("[ERROR] Git Remote URL이 비어있습니다. GIT_REMOTE_URL_WITH_TOKEN 을 설정하세요.")
            return

        commit_message = f"Auto update - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"

        self.append_log("[GIT] 업로드를 시작합니다...")

        # Thread 구성
        self._git_thread = QThread(self)
        self._git_worker = GitUploadWorker(repo_path, remote_url, branch, pull_first, commit_message)
        self._git_worker.moveToThread(self._git_thread)

        self._git_thread.started.connect(self._git_worker.run)
        self._git_worker.log.connect(self.append_log)
        self._git_worker.finished.connect(self._on_git_upload_finished)

        # 정리
        self._git_worker.finished.connect(self._git_thread.quit)
        self._git_worker.finished.connect(self._git_worker.deleteLater)
        self._git_thread.finished.connect(self._git_thread.deleteLater)

        # 버튼 잠깐 비활성화
        self.btn_upload_git.setEnabled(False)
        self._git_thread.start()

    def _on_git_upload_finished(self, ok: bool):
        if ok:
            self.append_log("[GIT] ✅ 업로드 성공")
        else:
            self.append_log("[GIT] ❌ 업로드 실패 (로그 확인)")

        self.btn_upload_git.setEnabled(True)
