...
# =========================================================
# TODO: 여기에서 3D 카메라 SDK import 하면 됨
# 예)
# from mechmind_sdk import Camera
# =========================================================
...
def depth_to_color_image(depth_np: np.ndarray) -> np.ndarray:
    ...
    # 가까운 곳(값 작음)을 파란색, 먼 곳(값 큼)을 빨간색으로
...
class MainWindow(QMainWindow):
    ...
    def on_refresh_clicked(self):
        self.append_log("[INFO] Refresh 클릭 (추후 기능 연결 예정)")
        if self.btn_point.isChecked() and self.last_pointcloud is not None:
            self.update_pointcloud_view()
        elif self.btn_depth.isChecked() and self.last_depth_qimage is not None:
            self.load_depth_sample(redraw_only=True)
    ...
    def scan_pointcloud(self):
        if not self.camera_connected and USE_REAL_CAMERA:
            self.append_log("[WARN] 카메라가 연결되지 않았습니다.")
            return

        self.append_log("[INFO] 스캔(또는 샘플 파일 로드) 시작...")

        try:
            if self.btn_point.isChecked():
                self.load_ply_sample()
            elif self.btn_depth.isChecked():
                self.load_depth_sample()
        except Exception as e:
            self.append_log(f"[ERROR] 스캔/파일 로드 중 오류: {e}")
