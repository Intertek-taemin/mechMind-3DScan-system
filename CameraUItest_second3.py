...
class MainWindow(QMainWindow):
    ...
    def compute_depth_diff_stats(self,
        ...
        if ref_depth is None or cur_depth is None:
            return float("inf"), float("inf"), 1.0, 10.0
        ...
        # 한쪽만 유효한 곳(= 한쪽은 물체, 한쪽은 빈 공간)은 "큰 차이"로 처리
        threshold_mm = 20.0  # 1cm
        only_one_valid = valid_any & (~both_valid)
        diff_mm[only_one_valid] = threshold_mm  # 또는 threshold_mm * 2 같은 값도 가능
    ...
    def make_diff_depth_qimage(self) -> QImage | None:
        ...
        # 크롭 후 diff 픽셀 수 다시 계산 (디버그용)
        num_valid = int(mask_valid_obj.sum())
        num_diff = int(mask_diff.sum())
        self.append_log(
