...
class ScanViewer(QWidget):
    ...
    def _update_ticks_from_points(self, pts_for_view: np.ndarray):
        ...
        # --- 카메라에 "가까운" 모서리(edge) 선택 ---
        edge_y = y0 if cam[1] < cy else y1   # X축 라벨/틱을 붙일 y edge
        edge_x = x0 if cam[0] < cx else x1   # Y축 라벨/틱을 붙일 x edge
        ...
        x_tick_y  = edge_y
        x_label_y = edge_y + x_out * (label_off * 2.0)

        y_tick_x  = edge_x
        y_label_x = edge_x + y_out * (label_off * 2.0)
        ...
        if self.gl_axis is not None:
            ...
            self.gl_axis.setSize(axis_len, axis_len, axis_len)
            self._reset_and_translate(self.gl_axis, x0, y0, z_plane)
        ...
        for i, zv in enumerate(zs):
            seg = np.array([[z_anchor_x, z_anchor_y, zv], [z_anchor_x + y_in * tick_len, z_anchor_y, zv]], dtype=np.float32)
            ln = GLLinePlotItem(pos=seg, mode="lines", antialias=True)
            self.gl_view.addItem(ln); self._tick_items.append(ln)

            txt = GLTextItem(
                pos=(float(z_label_x), float(z_label_y), float(zv + 3*z_eps)),
                text=f"{zv:.0f}",
                color=(220, 220, 220, 255),
                font=tick_font
            )
