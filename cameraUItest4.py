...
def depth_to_color_image(depth_np: np.ndarray) -> np.ndarray:
    ...
    # -------------------------------------------------
    # ★ 여기서 유효 깊이 영역만 자동 크롭해서 "줌인"
    # -------------------------------------------------
    ys, xs = np.where(mask)
    if ys.size > 0:
        h_img, w_img, _ = rgb_uint8.shape

        margin = 10  # 주변 여유 픽셀
        y_min = max(0, int(ys.min()) - margin)
        y_max = min(h_img - 1, int(ys.max()) + margin)
        x_min = max(0, int(xs.min()) - margin)
        x_max = min(w_img - 1, int(xs.max()) + margin)

        rgb_uint8 = rgb_uint8[y_min:y_max + 1, x_min:x_max + 1, :]
...
class MainWindow(QMainWindow):
    ...
    def capture_both_depth_and_point(self):
        ...
        if USE_REAL_CAMERA:
            ...
            # depth 컬러맵 QImage 생성 (왼쪽 Depths 모드에서 사용)
            ...
            pc_np = np.ascontiguousarray([pc.data()])   # 보통 (H,W,3) 또는 (N,3)
            if pc_np.ndim == 3:
