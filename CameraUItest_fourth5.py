...
FIXED_MAX_LARGEST_CLUSTER_PTS = 50
FIXED_MAX_LARGEST_BLOB_PX = 0
...
def add_metric_grid_with_labels(view, half=400, step=100, z=0):
    """
    view : GLViewWidget
    half : 원점 기준 표시 범위 (mm) -> -half ~ +half
    step : 눈금 간격 (mm)
    z    : 그리드 평면 높이 (기본 0)
    """
    # 이미 만들어진 오버레이가 있으면 중복 생성 방지
    if getattr(view, "_metric_items", None):
        return
    ...
    # 1) 그리드
    grid = GLGridItem()
    grid.setSize(half * 2, half * 2)
    grid.setSpacing(step, step)
    grid.translate(0, 0, z)
    view.addItem(grid)
    items.append(grid)

    # 2) 축(색 있는 XYZ 라인)
    axis = GLAxisItem()
    axis.setSize(x=half, y=half, z=half)
    view.addItem(axis)
    items.append(axis)

    # 3) 눈금(틱) + 숫자 라벨
    # 라벨을 격자 바깥쪽에 두면 오브젝트랑 덜 겹침
    label_y = -half  # X축 숫자 라벨을 y=-half 라인에 배치
    label_x = -half  # Y축 숫자 라벨을 x=-half 라인에 배치

    # 틱 길이(짧은 선)
    tick_len = step * 0.15

    for v in range(-half, half + 1, step):
        if v == 0:
            continue

        # X축 방향 눈금( y=label_y 근처에 짧은 선 )
        x_tick = np.array([[v, label_y, z], [v, label_y + tick_len, z]], dtype=float)
        x_tick_item = GLLinePlotItem(pos=x_tick, mode="lines", antialias=True)
        view.addItem(x_tick_item)
        items.append(x_tick_item)

        x_text = GLTextItem(pos=(v, label_y - tick_len * 1.2, z), text=f"{v}")
        view.addItem(x_text)
        items.append(x_text)

        # Y축 방향 눈금( x=label_x 근처에 짧은 선 )
        y_tick = np.array([[label_x, v, z], [label_x + tick_len, v, z]], dtype=float)
        y_tick_item = GLLinePlotItem(pos=y_tick, mode="lines", antialias=True)
        view.addItem(y_tick_item)
        items.append(y_tick_item)

        y_text = GLTextItem(pos=(label_x - tick_len * 1.8, v, z), text=f"{v}")
        view.addItem(y_text)
        items.append(y_text)

    # 나중에 제거/재구성할 수 있게 저장
    view._metric_items = items


def remove_metric_grid_with_labels(view):
    """스캔 갱신/리셋 시 오버레이 제거용"""
    items = getattr(view, "_metric_items", [])
    for it in items:
        try:
            view.removeItem(it)
        except Exception:
...
class ScanViewer(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # --- 좌표(축/수치) 오버레이 ---
        self.show_coords_overlay = False
        self.gl_axis = None
        self.gl_grid = None

        self.coord_overlay = QLabel(self)
        self.coord_overlay.setText("")
        self.coord_overlay.setStyleSheet(
            "background: rgba(0,0,0,150); color:white; padding:8px; "
            "border:1px solid rgba(255,255,255,60); border-radius:8px; "
            "font-size:12px;"
        )
        self.coord_overlay.hide()
        self._tick_items = []
        self.tick_divisions = 8
    ...
    def ensure_gl_view(self):
        ...
        # 좌표축/그리드 (기본 숨김)
        if self.gl_axis is None and GLAxisItem is not None:
            self.gl_axis = GLAxisItem()
            self.gl_axis.setSize(100, 100, 100)
            self.gl_axis.setVisible(False)
            self.gl_view.addItem(self.gl_axis)

        if self.gl_grid is None and GLGridItem is not None:
            self.gl_grid = GLGridItem()
            self.gl_grid.setSize(200, 200)
            self.gl_grid.setSpacing(20, 20)
            self.gl_grid.setVisible(False)
    ...
    def _clear_metric_items(self):
        if self.gl_view is None:
            self._metric_items = []
            return
        for it in self._metric_items:
            try:
    ...
    def _nice_step(self, span: float, target_ticks: int = 9) -> float:
        """span(mm)에 대해 1-2-5*10^n 형태의 보기 좋은 눈금 간격 선택"""
    ...
    def _update_metric_grid_and_ticks(self, pts_abs: np.ndarray):
        """✅ 공식 뷰어처럼: 그리드 + X/Y/Z 눈금(틱/숫자) 생성"""
        if self.gl_view is None or self.gl_grid is None or self.gl_axis is None:
            return

        # 기존 눈금 제거
        self._clear_metric_items()

        mins = np.nanmin(pts_abs, axis=0)
        maxs = np.nanmax(pts_abs, axis=0)

        span_x = float(maxs[0] - mins[0])
        span_y = float(maxs[1] - mins[1])
        span_z = float(maxs[2] - mins[2])

        # XY 그리드 스텝(너무 촘촘하면 자동으로 커짐)
        step_xy = self._nice_step(max(span_x, span_y), target_ticks=10)
        ...
        eps = step_xy * 0.002  # z-fighting 방지

        # --- 그리드 설정 (GLGridItem은 "중심 기준"이라 중앙으로 이동시켜야 함) ---
        grid_w = max(1.0, (x1 - x0))
        grid_h = max(1.0, (y1 - y0))
        self.gl_grid.setSize(grid_w, grid_h)
        self.gl_grid.setSpacing(step_xy, step_xy)

        grid_cx = (x0 + x1) * 0.5
        grid_cy = (y0 + y1) * 0.5
        self._reset_and_translate(self.gl_grid, grid_cx, grid_cy, z_plane - eps)

        # --- 축(AXIS)도 그리드 코너(또는 원하는 기준점)에 붙이기 ---
        axis_len = max(grid_w, grid_h, max(50.0, span_z)) * 0.25
        self.gl_axis.setSize(axis_len, axis_len, axis_len)
        self._reset_and_translate(self.gl_axis, x0, y0, z_plane - eps)

        # --- 틱/라벨 생성 ---
        tick_len = step_xy * 0.15
        label_off = tick_len * 1.4
    ...
    def _reset_and_translate(self, item, x, y, z):
        try:
            item.resetTransform()  # 누적 변환 방지
        except Exception:
            pass
        try:
    ...
    def _update_ticks_from_points(self, pts_for_view: np.ndarray):
        """
        ✅ pts_for_view(= scatter에 넣는 좌표) 기준으로:
        min/max → N등분 → 틱선 + 숫자(GLTextItem) 표시
        """
        try:
            from PySide6.QtGui import QFont
        ...
        pts = np.asarray(pts_for_view, dtype=np.float32)
        if pts.size == 0:
            return
        ...
        try:
            ...
            cam = np.array([float(cp.x()), float(cp.y()), float(cp.z())], dtype=float)
        except Exception:
        ...
        edge_x = x0 if abs(cam[0] - x0) < abs(cam[0] - x1) else x1
        ...
        if self.gl_grid is not None:
            grid_w = float(maxs[0] - mins[0])
            grid_h = float(maxs[1] - mins[1])
            grid_w = max(grid_w, 1.0)
            grid_h = max(grid_h, 1.0)

            self.gl_grid.setSize(grid_w, grid_h)

            # spacing은 "등분 간격"에 맞추기(가장 작은 간격을 사용)
            sx = float((maxs[0] - mins[0]) / div_xy) if div_xy > 0 else 10.0
            sy = float((maxs[1] - mins[1]) / div_xy) if div_xy > 0 else 10.0
            spacing = max(1.0, min(sx, sy))
            self.gl_grid.setSpacing(spacing, spacing)

            # GLGridItem은 중심기준이라 중앙으로 translate
            cx = float((mins[0] + maxs[0]) * 0.5)
            cy = float((mins[1] + maxs[1]) * 0.5)
            self._reset_and_translate(self.gl_grid, cx, cy, z_plane)

        # --- (선택) 축도 같은 기준점으로 이동 ---
        if self.gl_axis is not None:
            axis_len = max(50.0, base * 0.5)
            self.gl_axis.setSize(axis_len, axis_len, axis_len)
            self._reset_and_translate(self.gl_axis, edge_x, edge_y, z_plane)
        ...
        cam_xy = np.array([cam[0] - z_anchor_x, cam[1] - z_anchor_y], dtype=float)
        n_xy = float(np.linalg.norm(cam_xy))
        if n_xy < 1e-6:
            cam_xy = np.array([y_out, x_out], dtype=float)
            n_xy = float(np.linalg.norm(cam_xy))
        out_xy = np.array([cam_xy[0] / n_xy, cam_xy[1] / n_xy, 0.0], dtype=float)

        z_ruler_x, z_ruler_y = z_anchor_x, z_anchor_y  # ✅ 축(코너)에 붙임

        for i, zv in enumerate(zs):
            # 첫 값(바닥)은 X/Y 라벨과 겹치기 쉬워 생략(원하면 주석 해제)
            if i == 0:
                continue
            ...
            txt_pos = (
                float(z_ruler_x + out_xy[0] * (tick_len + label_off * label_mul)),
                float(z_ruler_y + out_xy[1] * (tick_len + label_off * label_mul)),
                float(zv + 3.0 * z_eps),
            )
            txt = GLTextItem(
                pos=txt_pos,
        ...
        # 텍스트는 "카메라 쪽"으로 오프셋(예전처럼 자연스러운 방향)
        cam_xy = np.array([cam[0] - z_ruler_x, cam[1] - z_ruler_y], dtype=float)
        n = np.linalg.norm(cam_xy)
        if n < 1e-6:
            cam_xy = np.array([1.0, 1.0], dtype=float)
            n = np.linalg.norm(cam_xy)
        cam_xy /= n

        for i, zv in enumerate(zs):
            # ✅ 바닥(z0) 라벨은 X/Y 원점 라벨과 자주 겹치니 보통 생략하는 게 자연스러움
            if i == 0:
                continue

            p0 = np.array([z_ruler_x, z_ruler_y, zv], dtype=np.float32)
            p1 = np.array([z_ruler_x + in_xy[0] * tick_len,
                        z_ruler_y + in_xy[1] * tick_len,
                        zv], dtype=np.float32)
            seg = np.vstack([p0, p1])
            ...
            # 라벨은 ruler에서 조금 더 바깥쪽으로
            txt_pos = (
                float(z_ruler_x + cam_xy[0] * (label_off * 1.0)),
                float(z_ruler_y + cam_xy[1] * (label_off * 1.0)),
                float(zv + 3.0 * z_eps),
            )
    ...
    def show_pointcloud(self, points: np.ndarray, colors: np.ndarray | None = None):
        ...
        if pts.size == 0:
            self.label.setText("유효 포인트가 없습니다.(NaN/Inf만 존재)")
            self.label.show()
            if self.gl_view is not None:
                self.gl_view.hide()
            return
        ...
        axis_len = max(50.0, radius * 1.2)  # 너무 작지 않게
        if self.gl_axis is not None:
            self.gl_axis.setSize(axis_len, axis_len, axis_len)

        if self.gl_grid is not None:
            grid_size = axis_len * 2.0
            spacing = max(5.0, axis_len / 10.0)
            # resetTransform이 없을 수 있어도 setSize/Spacing은 안전
            self.gl_grid.setSize(grid_size, grid_size)
            self.gl_grid.setSpacing(spacing, spacing)
    ...
    def show_qimage(self, qimg: QImage | None, empty_text: str = "스캔 데이터 출력"):
        ...
        # 이미지/뎁스 모드에서는 좌표/축 숨김
        if self.gl_axis is not None:
            self.gl_axis.setVisible(False)
        if self.gl_grid is not None:
            self.gl_grid.setVisible(False)
        if self.coord_overlay is not None:
            self.coord_overlay.hide()
    ...
    def set_show_coords(self, enabled: bool):
        self.show_coords_overlay = bool(enabled)
        self._apply_coords_visibility()
    ...
    def _apply_coords_visibility(self):
        ...
        if self.gl_axis is not None:
            self.gl_axis.setVisible(on)
        if self.gl_grid is not None:
            self.gl_grid.setVisible(on)

        # 텍스트가 있을 때만 라벨 표시
        if self.coord_overlay is not None:
            if on and (self.coord_overlay.text().strip() != ""):
    ...
    def _update_coord_text(self, pts_centered: np.ndarray):
        if self.coord_overlay is None:
            return
        if pts_centered is None or pts_centered.size == 0:
            self.coord_overlay.setText("")
            return

        mins = np.nanmin(pts_centered, axis=0)
        maxs = np.nanmax(pts_centered, axis=0)
        size = maxs - mins

        text = (
            f"Points: {pts_centered.shape[0]:,}\n"
            f"X(mm): {mins[0]:.2f} ~ {maxs[0]:.2f}   (Δ {size[0]:.2f})\n"
            f"Y(mm): {mins[1]:.2f} ~ {maxs[1]:.2f}   (Δ {size[1]:.2f})\n"
            f"Z(mm): {mins[2]:.2f} ~ {maxs[2]:.2f}   (Δ {size[2]:.2f})"
        )
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setStyleSheet("""
            ...
            QPushButton {
                background:#4a6c7a; color:white; padding:8px 10px; border-radius:8px;
                font-size:13px;
            }
            QPushButton:checked { background:#0e5a7a; }
            QPushButton:disabled { background:#3f4c55; }
        """)
    ...
    def _build_toolbar(self):
        ...
        # --- NEW: 3D 좌표 표시 토글(Recent 헤더에 배치) ---
        self.btn_xyz_toggle = QPushButton("XYZ OFF")
        self.btn_xyz_toggle.setCheckable(True)
        self.btn_xyz_toggle.setToolTip("3D 좌표(축/수치) 표시 토글")
        self.btn_xyz_toggle.clicked.connect(self.on_xyz_toggle_clicked)
        ...
        # --- View Mode ---
        self.btn_point.setIcon(self._icon_cube())
        self.btn_depth.setIcon(self._icon_depth_polyline())
        self.btn_image.setIcon(self._icon_landscape())

        self.btn_point.setIconSize(QSize(18, 18))
    ...
    def _build_central_views(self):
        ...
        # ✅ GroupBox를 멤버로 잡아두면, _sync_top_labels()에서 타이틀 갱신 가능
        self.gb_compare = QGroupBox("저장된 비교 대상")
        left_layout = QVBoxLayout(self.gb_compare)

        # ✅ Compare 패널 헤더: 라벨 삭제 → 버튼만 우측 정렬
        ...
        self.gb_recent = QGroupBox("가장 최근에 촬영된 이미지")
        right_layout = QVBoxLayout(self.gb_recent)

        # ✅ Recent 패널 헤더: 라벨 삭제 → 버튼만 우측 정렬
        ...
        splitter.addWidget(self.gb_compare)
        splitter.addWidget(self.gb_recent)
    ...
    def _build_settings_dock(self):
        ...
        # ✅ (변경) 너무 큰 박스 → 좌측 고정 크기로 축소
        self.label_tol_indicator.setFixedSize(120, 80)
        self.label_tol_indicator.setStyleSheet(
            "background:white; color:black; font-size:34px; font-weight:bold; border-radius:10px;"
        )

        # ✅ (추가) 상세 정보 라벨(오른쪽 공간)
        self.label_tol_details = QLabel("판정 세부 정보가 여기에 표시됩니다.")
        self.label_tol_details.setWordWrap(True)
        self.label_tol_details.setAlignment(Qt.AlignVCenter | Qt.AlignLeft)
        self.label_tol_details.setMinimumHeight(80)
        self.label_tol_details.setStyleSheet(
            "background:#1f1f1f; color:white; border:1px solid #555; border-radius:10px; "
    ...
    def _icon(self, theme_name: str, fallback_sp: QStyle.StandardPixmap) -> QIcon:
        """테마 아이콘 우선, 없으면 Qt 표준 아이콘 사용."""
    ...
    def _mk_pixmap(self, size: int = 18) -> QPixmap:
        pm = QPixmap(size, size)
        pm.fill(Qt.transparent)
        return pm

    def _stroke_color(self):
        # 버튼/테마에 맞춰 자동으로 잘 보이는 색(대체로 흰색/밝은색)
        return self.palette().color(QPalette.ButtonText)

    def _icon_cube(self, size: int = 18) -> QIcon:
        pm = self._mk_pixmap(size)
        p = QPainter(pm)
        p.setRenderHint(QPainter.Antialiasing, True)

        col = self._stroke_color()
        pen = QPen(col, 1.6)
        ...
        pad = 3
        off = 4  # isometric offset
        w = size - pad * 2 - off

        # back square
        back = QRectF(pad, pad, w, w)
        # front square (shifted)
        front = QRectF(pad + off, pad + off, w, w)
    ...
    def _icon_depth_polyline(self, size: int = 18) -> QIcon:
        pm = self._mk_pixmap(size)
        p = QPainter(pm)
        p.setRenderHint(QPainter.Antialiasing, True)
        ...
        # axes (L shape)
        p.drawLine(QPointF(left, top + 2), QPointF(left, bottom))
        p.drawLine(QPointF(left, bottom), QPointF(right, bottom))

        # polyline graph
        pts = [
            QPointF(left + 2, bottom - 2),
            QPointF(left + 5, bottom - 7),
            QPointF(left + 8, bottom - 5),
            QPointF(left + 11, bottom - 11),
            QPointF(left + 13, bottom - 9),
            QPointF(right - 1, bottom - 14),
        ]
        p.drawPolyline(pts)
    ...
    def _icon_landscape(self, size: int = 18) -> QIcon:
        pm = self._mk_pixmap(size)
        p = QPainter(pm)
        p.setRenderHint(QPainter.Antialiasing, True)
        ...
        p.drawEllipse(QPointF(rect.right() - 4.5, rect.top() + 4.5), 1.5, 1.5)

        # mountains (outline)
        p.setBrush(Qt.NoBrush)
        path = QPainterPath()
        path.moveTo(rect.left() + 2.0, rect.bottom() - 3.0)
        path.lineTo(rect.left() + 5.5, rect.bottom() - 7.5)
        path.lineTo(rect.left() + 8.0, rect.bottom() - 5.5)
        path.lineTo(rect.left() + 10.5, rect.bottom() - 9.0)
        path.lineTo(rect.right() - 2.0, rect.bottom() - 3.0)
        p.drawPath(path)

        p.end()
        return QIcon(pm)

    def _icon_diff_gradient(self, size: int = 18) -> QIcon:
        pm = self._mk_pixmap(size)
        p = QPainter(pm)
        p.setRenderHint(QPainter.Antialiasing, True)
        ...
        p.setPen(QPen(self._stroke_color(), 1.3))
        p.drawRoundedRect(rect, 2.5, 2.5)

        p.end()
        return QIcon(pm)

    def _icon_chain_link(self, size: int = 18) -> QIcon:
        pm = self._mk_pixmap(size)
        p = QPainter(pm)
        p.setRenderHint(QPainter.Antialiasing, True)
        ...
        # two rounded rectangles (links), slightly overlapped
        pad = 3.0
        w = size - pad * 2
        h = 6.0
        r = 3.0
        ...
        # link1 (left-up)
        rect1 = QRectF(pad + 0.5, pad + 4.0, w * 0.7, h)
        # link2 (right-down)
        rect2 = QRectF(pad + w * 0.3, pad + 1.5, w * 0.7, h)

        p.drawRoundedRect(rect1, r, r)
    ...
    def on_xyz_toggle_clicked(self):
        on = self.btn_xyz_toggle.isChecked()
        self.btn_xyz_toggle.setText("XYZ ON" if on else "XYZ OFF")

        # 두 뷰어 모두 적용
        self.scan_viewer.set_show_coords(on)
        self.compare_viewer.set_show_coords(on)

        # 현재 Point 모드면 즉시 갱신해서 반영(특히 이미 떠있는 뷰)
    ...
    def _sync_top_labels(self):
        ...
        if hasattr(self, "gb_compare") and self.gb_compare:
            self.gb_compare.setTitle(f"비교 데이터   (Compare: {cmp})")
        if hasattr(self, "gb_recent") and self.gb_recent:
            self.gb_recent.setTitle(f"최근 촬영 데이터   (Last: {last})")

        # 상태바에도 같이
        if self.statusBar():
            self.statusBar().showMessage(f"Compare: {cmp} | Last: {last}")
    ...
    def save_compare_bundle(self, folder_name: str):
        ...
        meta = {
            ...
            "tol_ratio_percent": float(FIXED_ALLOW_RATIO * 100.0),
        ...
        if os.path.isfile(meta_path):
            try:
                ...
                if ratio_pct is not None:
                    self.append_log(f"[COMPARE] META의 tol_ratio_percent={ratio_pct} 값은 무시합니다. (fixed={FIXED_ALLOW_RATIO*100:.1f}%)")
    ...
    def _largest_dbscan_cluster_size(self, pts_xyz: np.ndarray, eps_mm: float, min_points: int = 10) -> int:
        """
        outlier 점(3D)에서 DBSCAN으로 클러스터링 후,
        가장 큰 클러스터의 '점 개수'를 반환.
        """
        if o3d is None:
            return 0
        if pts_xyz is None:
        ...
        labels = np.array(pcd.cluster_dbscan(eps=float(eps_mm), min_points=int(min_points), print_progress=False))
        if labels.size == 0:
            return 0
        ...
        counts = np.bincount(labels[valid].astype(np.int32))
        return int(counts.max()) if counts.size > 0 else 0

    def _largest_connected_component_area(self, mask: np.ndarray) -> int:
        """
        2D bool mask에서 가장 큰 연결요소(Blob)의 픽셀 수를 반환.
        가능하면 cv2.connectedComponentsWithStats 사용(빠름).
        """
        if mask is None:
            return 0
        ...
        if cv2 is not None:
            ...
            if num <= 1:
                return 0
            # stats: [label, x, y, w, h, area] / 0은 background
            areas = stats[1:, cv2.CC_STAT_AREA]
            return int(areas.max()) if areas.size > 0 else 0

        # fallback(느림): cv2 없을 때는 그냥 "클러스터 판단 생략"
        return int(mask.sum())
    ...
    def compute_depth_diff_stats(self,
        ...
        # (참고용) 기존 방식 전체 diff 픽셀 비율
        overall_ratio = float(mask_diff.sum() / (valid_any.sum() + 1e-6))

        # ★ 클러스터(Blob) 방식: 가장 큰 blob 면적만 사용
        largest_blob = self._largest_connected_component_area(mask_diff)
        cluster_ratio = float(largest_blob / (valid_any.sum() + 1e-6))

        self.append_log(
            f"[DEBUG] (DepthCluster) overall_ratio={overall_ratio*100:.2f}%, "
            f"largest_blob={largest_blob}px, blob_ratio={cluster_ratio*100:.2f}% (thr={threshold_mm:.1f}mm)"
        )

        self._last_depth_min_mm = min_mm
        self._last_depth_mean_mm = mean_mm
        self._last_depth_max_mm = max_mm
        self._last_depth_overall_ratio = overall_ratio
        self._last_depth_cluster_ratio = cluster_ratio
        self._last_depth_largest_blob = int(largest_blob)

        mean_cm = mean_mm / 10.0
        max_cm = max_mm / 10.0

        # 반환 diff_ratio 자리에 blob 기반 cluster_ratio를 넣는다
        return mean_cm, max_cm, cluster_ratio, threshold_mm
    ...
    def compute_pointcloud_diff_stats(self,
        ...
        # --- ★ 클러스터 방식: outlier 점들을 모아 가장 큰 클러스터 비율만 사용 ---
        pts_cur = np.asarray(pcd_cur_aligned.points, dtype=np.float32)
        pts_ref = np.asarray(pcd_ref.points, dtype=np.float32)

        out_cur = pts_cur[d_cur_to_ref >= threshold_mm] if pts_cur.size else np.empty((0, 3), np.float32)
        out_ref = pts_ref[d_ref_to_cur >= threshold_mm] if pts_ref.size else np.empty((0, 3), np.float32)

        if out_cur.size == 0 and out_ref.size == 0:
            largest_cluster_pts = 0
            cluster_ratio = 0.0
        else:
            out_pts = out_cur if out_ref.size == 0 else (out_ref if out_cur.size == 0 else np.vstack([out_cur, out_ref]))

            # eps는 voxel_size(2mm) 기준으로 너무 작으면 쪼개짐 → 최소 6mm 권장
            eps_mm = max(6.0, threshold_mm * 1.5)
            largest_cluster_pts = self._largest_dbscan_cluster_size(out_pts, eps_mm=eps_mm, min_points=10)

            denom = float(d_cur_to_ref.size + d_ref_to_cur.size) + 1e-6
            cluster_ratio = float(largest_cluster_pts / denom)

        self.append_log(
            f"[DEBUG] (PointCluster) overall_ratio={overall_ratio*100:.2f}%, "
            f"largest_cluster={largest_cluster_pts}pts, cluster_ratio={cluster_ratio*100:.2f}% (thr={threshold_mm:.1f}mm)"
        )
        ...
        self._last_point_cluster_ratio = cluster_ratio
        self._last_point_largest_cluster_pts = largest_cluster_pts
    ...
    def get_allow_ratio(self) -> float:
        # ✅ UI 입력값 무시, 코드에서 고정
        return float(FIXED_ALLOW_RATIO)
    ...
    def update_tolerance_display(self):
        ...
        if src == "Point":
            ...
            largest_val = int(getattr(self, "_last_point_largest_cluster_pts", 0))
            max_largest = self.get_max_largest_cluster_pts()
        else:
            min_mm_disp  = float(getattr(self, "_last_point_min_mm", 0.0))
            mean_mm_disp = float(getattr(self, "_last_point_mean_mm", mean_cm * 10.0))
            max_mm_disp  = float(getattr(self, "_last_point_max_mm",  max_cm * 10.0))
            largest_val = int(getattr(self, "_last_depth_largest_blob", 0))
            max_largest = self.get_max_largest_blob_px()

        largest_ok = True if (max_largest is None) else (largest_val <= max_largest)

        # ✅ (추가) 상세표시 문자열
        if max_largest is None:
            extra = f"largest={largest_val} (cap=OFF)"
        else:
            extra = f"largest={largest_val}/{max_largest}" + (" ✅" if largest_ok else " ❌")
        
        # ✅ 상세 라벨 갱신
        if hasattr(self, "label_tol_details") and self.label_tol_details:
            self.label_tol_details.setText(
                f"소스: {src}\n"
                f"기준: 거리 {thr_mm:.1f} mm / 비율 {ratio_limit*100:.2f}%\n"
                f"오차(mm): min {min_mm_disp:.2f} / mean {mean_mm_disp:.2f} / max {max_mm_disp:.2f}\n"
                f"클러스터 비율: {diff_ratio*100:.2f}%  ({extra})"
            )
        ...
        if (mean_cm < thr_cm) and (diff_ratio < ratio_limit) and largest_ok:
    ...
    def _compute_judgement_stats(self):
        ...
        if o3d is not None and self.compare_pointcloud is not None and self.last_pointcloud is not None:
            ...
            if pc is not None:
                mean_cm, max_cm, cluster_ratio, thr_mm = pc
                ...
                largest_pts = int(getattr(self, "_last_point_largest_cluster_pts", 0))
                max_largest = self.get_max_largest_cluster_pts()
                largest_ok = True if (max_largest is None) else (largest_pts <= max_largest)

                verdict = "V" if (mean_mm < thr_mm) and (cluster_ratio < limit_ratio) and largest_ok else "X"
                ...
                return {
                    ...
                    "mean_mm": mean_mm,
                    "max_mm": max_mm,
                    "ratio": float(cluster_ratio),   # ✅ 이제 '최대 클러스터 비율'
                    "thr_mm": float(thr_mm),
        ...
        if self.compare_depth_array is not None and self.last_depth_array is not None:
            ...
            if depth is not None:
                mean_cm, max_cm, blob_ratio, thr_mm = depth
                ...
                verdict = "V" if (mean_mm < thr_mm) and (blob_ratio < limit_ratio) else "X"
                return {
                    "src": "Depth",
                    "min_mm": min_mm,
                    "mean_mm": mean_mm,
                    "max_mm": max_mm,
                    "ratio": float(blob_ratio),      # ✅ 이제 '최대 blob 비율'
                    "thr_mm": float(thr_mm),
                    "limit_ratio": float(limit_ratio),
                    "verdict": verdict,
                }
    ...
    def get_max_largest_cluster_pts(self) -> int | None:
        v = int(FIXED_MAX_LARGEST_CLUSTER_PTS)
        return None if v <= 0 else v

    def get_max_largest_blob_px(self) -> int | None:
        v = int(FIXED_MAX_LARGEST_BLOB_PX)
        return None if v <= 0 else v
