...
class MainWindow(QMainWindow):
    ...
    def compute_pointcloud_diff_stats(self,
        ...
        기존: cur -> ref 한 방향 거리만 사용
        개선: cur -> ref, ref -> cur 양방향 거리 모두 사용
        """
        if o3d is None:
    ...
    def make_pointcloud_diff_for_view(self):
        ...
        개선점:
        - cur→ref 거리로 현재 포인트 색상 지정
        - ref→cur 거리가 큰 기준 포인트는 '추가 포인트(흰색)'로 붙여서
          안으로 들어간(깎인) 부분도 보이게 함
        """
        if o3d is None:
        ...
        # --- 4-1) 현재 포인트 색상: 0~thr 범위에서 녹색~빨강, thr 이상은 흰색 ---
        dist_clipped = np.clip(d_cur_to_ref, 0.0, threshold_mm)
        norm = dist_clipped / (threshold_mm + 1e-6)

        # 가까움(0) = 초록, 임계치 근처(1) = 빨강
        h = (1.0 - norm) * (1.0 / 3.0)  # green(1/3) ~ red(0)
        s = np.ones_like(h, dtype=np.float32)
        v = np.ones_like(h, dtype=np.float32)
        ...
        colors_cur[mask_large_cur] = 1.0  # 흰색

        # --- 4-2) 기준 포인트 중에서 '깎인 부분' 찾기: ref→cur 거리가 큰 점 ---
        mask_large_ref = d_ref_to_cur >= threshold_mm
        pts_ref_far = pts_ref[mask_large_ref]

        # 이 포인트들은 "있어야 할데 없는 곳"이므로 흰색 유령점으로 표시
        if pts_ref_far.size > 0:
            colors_ref_far = np.ones_like(pts_ref_far, dtype=np.float32)  # 전부 흰색
            # 현재 포인트 + 유령 기준 포인트 합치기
